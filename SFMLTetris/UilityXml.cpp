...
WQuestion * UilityXml::LoadXmlFile(const char * path)
{
	...
	return wquestionArray;
}

string UilityXml::Utf8ToUnicode(const char * szU8)
{
	//预转换，得到所需空间的大小;
...
string UilityXml::Wchar_tToString(wchar_t * wchar)
{
	wchar_t * wText = wchar;
...
wstring UilityXml::Utf8ToUnicodeW(const char * szU8)
{
	//预转换，得到所需空间的大小;
	int wcsLen = ::MultiByteToWideChar(CP_UTF8, NULL, szU8, strlen(szU8), NULL, 0);
